...
if [ $(echo $?) -eq 0 ]; then
    ...
    if [ "$critical" != null ] || [ "$high" != null ]; then
        ...
        echo $scan_results
        echo "Repo: $repository_name, image_tag: $image_tag, scan_results: $scan_results" >> ecr-image-scan-results-$image_tag.out
        ...
        echo "Repo: $repository_name, image_tag: $image_tag, scan_results: No critical or high level vulnerabilities found" >> ecr-image-scan-results-$image_tag.out
    fi
fi